...
from model_jjy import WaveDecoder, WaveEncoder
...
class Trainer():
    def __init__(self, loader, config):
        ...
        self.wavenc_freeze = config.wavenc_freeze
    ...
    def build_models(self):
        ...
        self.wave_encoder = WaveEncoder().to(self.device)
        self.wave_decoder = WaveDecoder().to(self.device)
        # Warmstart
        ...
        if self.wavenc_freeze:
            for param in self.wave_encoder.parameters():
                param.requires_grad = False
        else:            
    ...
    def train(self):
        ...
        with torch.no_grad():
            fixed_feature, fixed_skips_wave = self.wave_encoder(fixed_sample)
            fixed_recon_image, _ = self.wave_decoder(fixed_feature, fixed_skips_wave)
        ...
        for epoch in range(self.epoch):
            for num, real_image in enumerate(self.loader):
                ...
                feature_4_1, skips_wave = self.wave_encoder(real_image)
                recon_image, skips_wave = self.wave_decoder(feature_4_1, skips_wave)
                _, skips_wave_re = self.wave_encoder(recon_image)
                ...
                if 'perceptual' in self.loss_option:                    
                    ...
                    feature_loss += self.MSE_loss(skips_wave_re['conv3_1'], skips_vgg['conv3_1'])
                    feature_loss += self.MSE_loss(skips_wave_re['conv2_1'], skips_vgg['conv2_1'])
                    feature_loss += self.MSE_loss(skips_wave_re['conv1_1'], skips_vgg['conv1_1'])
                if 'gram' in self.loss_option:
                    feature_loss += self.MSE_loss(gram_matrix(feature_4_1), gram_matrix(feature_vgg_4_1))                    
                    feature_loss += self.MSE_loss(gram_matrix(skips_wave_re['conv3_1']), gram_matrix(skips_vgg['conv3_1']))
                    feature_loss += self.MSE_loss(gram_matrix(skips_wave_re['conv2_1']), gram_matrix(skips_vgg['conv2_1']))
                    feature_loss += self.MSE_loss(gram_matrix(skips_wave_re['conv1_1']), gram_matrix(skips_vgg['conv1_1']))
                
                if 'mirror_per' in self.loss_option:
                    mirror_feature_loss += self.MSE_loss(skips_wave['conv3_2_wavrecon'], skips_wave['conv3_1'])
                    mirror_feature_loss += self.MSE_loss(skips_wave['conv2_2_wavrecon'], skips_wave['conv2_1'])
                    mirror_feature_loss += self.MSE_loss(skips_wave['conv1_2_wavrecon'], skips_wave['conv1_1'])                    
...
#                     mirror_feature_loss += self.MSE_loss(skips_wave['conv2_1_wavrecon'], skips_wave['conv1_2'])                  
                if 'mirror_gram' in self.loss_option:
                    mirror_feature_loss += self.MSE_loss(gram_matrix(skips_wave['conv3_2_wavrecon']), gram_matrix(skips_vgg['conv3_1']))
                    mirror_feature_loss += self.MSE_loss(gram_matrix(skips_wave['conv2_2_wavrecon']), gram_matrix(skips_vgg['conv2_1']))
                    mirror_feature_loss += self.MSE_loss(gram_matrix(skips_wave['conv1_2_wavrecon']), gram_matrix(skips_vgg['conv1_1']))
                    ...
                    if (num + 1) % 500 == 0:
                        ...
                        torch.save(self.wave_encoder.state_dict(),
                                    os.path.join(self.model_save_path, 
                                                 "wave_encoder_{}_{}_{:.4f}_{:.4f}.pth".format(epoch+1, num+1, recon_loss.item(), feature_loss.item()))
                        )
                        torch.save(self.wave_decoder.state_dict(),
                                    os.path.join(self.model_save_path, 
                                                 "wave_decoder_{}_{}_{:.4f}_{:.4f}.pth".format(epoch+1, num+1, recon_loss.item(), feature_loss.item()))
                        )

        torch.save(self.wave_encoder.state_dict(),
                    os.path.join(self.model_save_path, "wave_encoder.pth")
        )
        torch.save(self.wave_decoder.state_dict(),
                    os.path.join(self.model_save_path, "wave_decoder.pth")
        )
